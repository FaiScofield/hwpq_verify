...
import json
import random
from .module_config_core import *
...
class CfaConfig(ModuleConfigCore):
    ...
    def gen(self, seed: int = 114514):
        ...
        if seed != None:
            ...
            seed = self.randSeed + 1
        random.seed(seed)

        self.version = f'RKCFA_0.13.0.4721.random_seed_{seed}'
        self.nCallCnt = 0
        self.nFrameIdx = 0
        self.nImgWid = random.randint(4, 4001) // 4 * 4 # align to 4
        self.nImgHgt = random.randint(2, 4001)  # no align
        ...
        self.ePlatform = random.randint(0, 11) # [0, 10]
        self.eCfaPattern = 0
        self.eAlgoType = random.randint(0, 3) # [0, 2]
        self.eImgFormat = 0
        self.eOutFormat = random.randint(11, 14) # [11, 13]
        self.eDisplayMode = 0
        self.nColorDepth = 64
        ...
        self.bDither = random.randint(0, 2) # [0, 1]
        self.bDeFalseColor4Gray = random.randint(0, 2) # [0, 1]
        self.bContrastEqual = 0
        self.bForceRunWithCpu = random.randint(0, 2) # [0, 1]
        self.nRegalType = 0
        self.nA2AlgoType = 2
        self.nA2CompLevel = random.randint(0, 80) # [0, 64]
        self.bA2Modulate = random.randint(0, 8) # [0, 7]
        self.bClearLow4bits = random.randint(0, 2) # [0, 1]
        self.sRoiInfo = [0, 0, 0, 0, 0, 0] # x6
        self.aReserved = [0, 0, 0, 0, 0, 0, 0, 0] # x8
        return seed
