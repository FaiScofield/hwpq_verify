...
typedef struct { float H, S, V; } hsv_f;   /* H∈[0,360), S,V∈[0,1] */
...
static inline hsv_f rgb2hsv_float(float r, float g, float b)
{
    ...
    if (C > 0.0f) {
        float hp;
        if (M == r)      hp = fmodf((g - b) / C, 6.0f);
        else if (M == g) hp = (b - r) / C + 2.0f;
        else             hp = (r - g) / C + 4.0f;
        if (hp < 0.0f) hp += 6.0f;
        o.H = 60.0f * hp;
    }
...
/* HSV -> RGB(0..1) */
static inline void hsv2rgb_float(float H, float S, float V,
