 ...
 * @note: Base on 'rockchip_post_csc.c' from the 'drm' project.
...
static const union csc_matrix_f32 g_y2r_mat_bt601_f32 = {1.f, 0.f, 1.402f, 1.f, -0.344136f, -0.714136f, 1.f, 1.772f, 0.f};
...
static const union csc_matrix_f32 g_y2r_mat_bt709_f32 = {1.f, 0.f, 1.5748f, 1.f, -0.187324f, -0.468124f, 1.f, 1.8556f, 0.f};
...
static const union csc_matrix_f32 g_y2r_mat_bt2020_f32 = {1.f, 0.f, 1.4746f, 1.f, -0.164553f, -0.571353f, 1.f, 1.8814f, 0.f};
...
static void csc_get_range_conversion_matrix_offset(const struct post_csc_convert_mode *mode, union csc_matrix_f32 *range_mat_i,
    union csc_matrix_f32 *range_mat_o, union csc_vector_s32 *offset_vec_i, union csc_vector_s32 *offset_vec_o)
{
...
static void csc_adjust_convert_matrix(const struct post_csc_convert_mode *mode, const struct post_csc *bcsh_cfg,
    union csc_matrix_f32 *out_mat, union csc_vector_s32 *out_vec)
{
        ...
        for (int i = 0; i < 3; ++i) {
            if (row_sums[i] != denorms[i]) {
                ...
                printf("NOTE: fine-tuning CSC coef[%d][%d] = %d => %d, since row_sums[%d] = %d != %d, delta = %d.\n", i,
                    col, fixed_mat->val[i][col], fixed_mat->val[i][col] + delta, i, row_sums[i], denorms[i], delta);
...
static const int g_hue_sin_table_10bit[256] = {512, 508, 505, 501, 497, 494, 490, 486, 483, 479, 475, 472, 468, 464, 460,
    ...
    277, 273, 269, 265, 261, 257, 253, 249, 245, 241, 237, 233, 228, 224, 220, 216, 212, 208, 204, 200, 196, 192, 187,
    183, 179, 175, 171, 167, 163, 159, 154, 150, 146, 142, 138, 134, 130, 125, 121, 117, 113, 109, 105, 100, 96, 92, 88,
    84, 80, 75, 71, 67, 63, 59, 54, 50, 46, 42, 38, 34, 29, 25, 21, 17, 13, 8, 4, 0, -4, -8, -13, -17, -21, -25, -29, -34,
    -38, -42, -46, -50, -54, -59, -63, -67, -71, -75, -80, -84, -88, -92, -96, -100, -105, -109, -113, -117, -121, -125,
    -130, -134, -138, -142, -146, -150, -154, -159, -163, -167, -171, -175, -179, -183, -187, -192, -196, -200, -204,
    -208, -212, -216, -220, -224, -228, -233, -237, -241, -245, -249, -253, -257, -261, -265, -269, -273, -277, -281,
    -285, -289, -293, -297, -301, -305, -309, -313, -317, -321, -325, -329, -333, -337, -341, -345, -349, -353, -357, -361,
    -365, -369, -372, -376, -380, -384, -388, -392, -396, -400, -403, -407, -411, -415, -419, -423, -426, -430, -434, -438,
    -442, -445, -449, -453, -457, -460, -464, -468, -472, -475, -479, -483, -486, -490, -494, -497, -501, -505, -508};

/* 10bit Hue Cos Look Up Table  -> range[-30, 30]. value range: [887, 1024] */
static const int g_hue_cos_table_10bit[256] = {887, 889, 891, 893, 895, 897, 899, 901, 903, 905, 907, 909, 911, 913,
    915, 917, 919, 920, 922, 924, 926, 928, 929, 931, 933, 935, 936, 938, 940, 941, 943, 945, 946, 948, 949, 951, 953,
    954, 956, 957, 959, 960, 962, 963, 964, 966, 967, 969, 970, 971, 973, 974, 975, 976, 978, 979, 980, 981, 983, 984,
    985, 986, 987, 988, 989, 990, 992, 993, 994, 995, 996, 997, 998, 998, 999, 1000, 1001, 1002, 1003, 1004, 1005, 1005,
    1006, 1007, 1008, 1008, 1009, 1010, 1011, 1011, 1012, 1013, 1013, 1014, 1014, 1015, 1015, 1016, 1016, 1017, 1017,
    1018, 1018, 1019, 1019, 1020, 1020, 1020, 1021, 1021, 1021, 1022, 1022, 1022, 1022, 1023, 1023, 1023, 1023, 1023,
    1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1023, 1023, 1023,
    1023, 1023, 1022, 1022, 1022, 1022, 1021, 1021, 1021, 1020, 1020, 1020, 1019, 1019, 1018, 1018, 1017, 1017, 1016,
    1016, 1015, 1015, 1014, 1014, 1013, 1013, 1012, 1011, 1011, 1010, 1009, 1008, 1008, 1007, 1006, 1005, 1005, 1004,
    1003, 1002, 1001, 1000, 999, 998, 998, 997, 996, 995, 994, 993, 992, 990, 989, 988, 987, 986, 985, 984, 983, 981,
    980, 979, 978, 976, 975, 974, 973, 971, 970, 969, 967, 966, 964, 963, 962, 960, 959, 957, 956, 954, 953, 951, 949,
    948, 946, 945, 943, 941, 940, 938, 936, 935, 933, 931, 929, 928, 926, 924, 922, 920, 919, 917, 915, 913, 911, 909,
    907, 905, 903, 901, 899, 897, 895, 893, 891, 889, 887};


static void csc_adjust_convert_matrix_fix(const struct post_csc_convert_mode *mode, const struct post_csc *bcsh_cfg,
...
static void csc_get_range_offset(const struct post_csc_convert_mode *mode, union csc_vector_s32 *offset_i,
    union csc_vector_s32 *offset_o)
{
    const int ratio_gain = 1 << (mode->pixel_depth - 8);
...
static void csc_swap_color_channel(const struct post_csc_convert_mode *mode, union csc_matrix_s32 *out_mat,
    union csc_vector_s32 *out_vec)
{
...
int csc_get_mode_index(const struct post_csc_convert_mode *mode)
{
    ...
    for (int i = 0; i < ARRAY_SIZE(g_supported_standard_convert_mode); ++i) {
        ...
        if (mode_i->intput_color_encoding == mode->intput_color_encoding &&
            ...
            mode_i->is_output_yuv == mode->is_output_yuv && mode_i->is_input_full_range == mode->is_input_full_range &&
