...
static void print_usage_addition(void)
{
    ...
    LOGI("      --cf_he_ratio        [val] | HE HE ratio, range: 5bit fixed, default follows json config\n");
...
int main(int argc, char *const argv[])
{
    ...
    for (int k = 0; k < config.nb_frame; k++) {
        if (is_src_stb_img) {
            ...
            ret = imgcvt_to_planar_10bit_lsb((uint8_t *)p_src_stb, (uint16_t *)p_src, config.src_wid, config.src_hgt,
        ...
        if (can_calc_crc) {
            crc_val = calc_crc32_rtl_10bit_planar(p_src, config.src_wid, config.src_hgt, config.src_wid * 2, bIsInputYuv);
        ...
        if (shp_type > 0) {
            ...
            p_final_out = p_shp;
            if (enable_dump_med_img) {
                ret = dump_image_file(config.output_dir, "dci_dbg4_output_shp", k, p_shp, config.dst_wid,
                    config.dst_hgt, config.dst_wid * 2, config.dst_hgt, YUV444P_10LSB, 10, config.dither_dn);
            }
        }
        ...
        if (is_dst_stb_img) {
            run_csc_with_coef_13bit(p_final_out, p_rgb, config.dst_wid, config.dst_hgt, &rgb_output_csc_coef,
                &rgb_output_csc_mode);
            if (enable_dump_med_img) {
                ret = dump_image_file(config.output_dir, "dci_dbg5_output_y2r", k, p_rgb, config.dst_wid,
        ...
        if (can_calc_crc) {
            dst_crc = calc_crc32_rtl_10bit_planar(p_final_out, config.dst_wid, config.dst_hgt, config.dst_wid * 2, bIsOutputYuv);
